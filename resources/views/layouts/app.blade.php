...
<body class="bg-slate-900 text-white">
    ...
    @if($env === 'local' || $env === 'development' || request()->getHost() === '127.0.0.1')
        <div id="devDebug"
            ...
            <div style="margin-top:6px;text-align:right"><button id="devDebugBtn"
                    style="background:#10b981;border:none;padding:4px 8px;border-radius:6px;color:#fff;cursor:pointer;font-size:12px">Run
                    startCamera()</button></div>
        </div>
        <script>
    ...
    @if(app()->environment('local') || request()->getHost() === '127.0.0.1')
        <div id="globalCamera" style="position:fixed;left:12px;bottom:12px;z-index:99999">
            <button id="openCameraTest"
                style="background:#0369a1;color:#fff;padding:8px 10px;border-radius:8px;border:none;font-size:13px;cursor:pointer">Camera
                Test</button>
        </div>
        <div id="cameraModal"
            style="display:none;position:fixed;left:0;top:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:100000;align-items:center;justify-content:center;">
            <div style="width:640px;max-width:95%;background:#111827;padding:16px;border-radius:8px;color:#fff;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    ...
                    <button id="closeCameraModal"
                        style="background:#ef4444;border:none;padding:6px 8px;border-radius:6px;color:#fff;cursor:pointer">Close</button>
                </div>
                <video id="globalCameraVideo" autoplay playsinline
                    style="width:100%;background:#000;border-radius:6px"></video>
                <div style="display:flex;gap:8px;margin-top:8px;justify-content:flex-end;">
                    <button id="globalCapture"
                        style="background:#10b981;border:none;padding:8px 10px;border-radius:6px;color:#fff;cursor:pointer">Capture</button>
                    <a id="globalDownload"
                        style="display:none;background:#0ea5e9;padding:8px 10px;border-radius:6px;color:#fff;text-decoration:none;">Download</a>
                </div>
            </div>
        </div>
        <script>
